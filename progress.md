...
## 探索点回退大模型选点、门缝与测地停止（2026-09-22）

探索模式：候选多于 1 个时大模型只选一次并锁定世界坐标，最多跟随 3 段；标注图只画编号，深度以 `{F1: 2.5m, …}` 文本传入；扇区半宽约 ±40° 重叠；画幅外但相机前方的点夹到内边距。近距候选以占用图测地 `< 5 m` 为准（见下节，已去掉直线通视过滤）。

门缝：路径上有门宽缩窄（净宽约 0.45–1.30 m）的簇放宽开口与外扩边界，最少 3 格即可，便于侧视门后一小块三角形地板仍抽出探索点；每朝向保留最近缩窄与最大簇。

Depth 工具主读数改为占用图测地 `geodesic_m`（反投影 + 落脚 + A*）；Stop 硬闸门用同一套数，不读评测 `distance_to_goal`。Look 落盘 `look1.png` 起；回合结束存 `final_obs.png`；无论状态，未发过停止则代发 `HAB_STOP` 结算指标。

单测：`python test/occupancy/test_frontier_reach.py`。

## 探索候选去掉直线通视（2026-09-22）

`20260922_104011` 约 87/100 次探索移动 `miss`：`frontier_in` 无绿点。根因是 `grid_line_clear` 把绕墙可达的点滤掉，扇区边缘点也进不了画面。

改动：扇区候选改为占用图测地有限且 `< 5 m`，不再要求机身到点直线不穿 `OCC`；`frontier_candidates` 只做夹边投影直接画。`grid_line_clear` 仍留给 Stop 同房间欧氏回退。流水首段写 `sector / near / drawn` 便于对照。
